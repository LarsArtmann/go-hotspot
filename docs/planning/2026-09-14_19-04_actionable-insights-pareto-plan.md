...
> ranked table, but zero _actionable_ insight — and silently analyzed the wrong repo when given a
> positional path.
...
**1% → 51% of the result**
Positional target directory + reject unknown arguments. _(Correctness: never silently analyze the wrong repo — this is the difference between insight and misinformation.)_
...
- Every task lands with build + tests green; goldens regenerated only when output change is _intended_ and reviewed in the diff.
