...
| Feature             | Status                | Notes                                                                                                                                                                                                                                                                                |
| ------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Table output        | 🟢 `FULLY_FUNCTIONAL` | Default format. Tabwriter-aligned with risk bands and relative SCORE (0–100 of max). Tested (unit + golden).                                                                                                                                                                         |
| Markdown output     | 🟢 `FULLY_FUNCTIONAL` | `--format markdown`. Pipe-table with backtick-wrapped paths and relative SCORE.                                                                                                                                                                                                      |
| CSV output          | 🟢 `FULLY_FUNCTIONAL` | `--format csv`. Full metrics incl. raw normalized hotspot for programmatic use.                                                                                                                                                                                                      |
| JSON output         | 🟢 `FULLY_FUNCTIONAL` | `--format json`. Summary metadata + hotspots + couplings + insights (+ functions when enabled).                                                                                                                                                                                      |
| Actionable insights | 🟢 `FULLY_FUNCTIONAL` | `hotspot.Insights` derives recommended actions from the result set: churn concentration, churn-without-complexity, complexity-without-churn, bus factor, stale hotspots, coupling advice. Rendered in table/markdown/JSON; `--no-insights` hides. Rules need ≥8 files for quartiles. |
...
| Feature                                | Status                | Notes                                                                                                                                                                                                                     |
| -------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Flag-driven analysis                   | 🟢 `FULLY_FUNCTIONAL` | 25+ flags for window, metrics, filtering, output, CI gates. Positional `[target]` directory argument. `cmd/go-hotspot/main.go`. 30+ tests (unit + integration with real git repos).                                       |
| File filtering                         | 🟢 `FULLY_FUNCTIONAL` | Extension list or `--ext auto` multi-language code profile, test toggle, generated detection (suffix + content-based `// Code generated` header), path prefix, vendor exclusion, `--min-commits`, `--author` (`main.go`). |
| Noise taming (`--verbose`)             | 🟢 `FULLY_FUNCTIONAL` | Files deleted from disk but present in git history are summarized as one count; real analysis failures warn per file. `--verbose` names every skipped path.                                                               |
| Strict enum validation                 | 🟢 `FULLY_FUNCTIONAL` | Typos in `--format`/`--sort`/`--complexity`/`--churn`/`--fail-risk` exit 1 with valid values listed instead of silently defaulting. Aliases preserved.                                                                    |
